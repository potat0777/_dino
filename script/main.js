...
function update(){
    ...
    world.objects.clouds.forEach((cloud)=>{
        ...
        if(cloudRect.left + cloudsRect.width - 20 <= cloudsRect){
    ...
    world.objects.roadLines.forEach((roadLine)=>{
        ...
        if(roadLineRect.left + roadLineRect.width - 10 <= roadRect.left){
            ...
            roadLine.location.x -= roadElement.velocity
        }
    })
    world.objects.foods.forEach((food, i )=>{
        ...
        if(foodRect.left + foodRect.endWith <= roadRect.left){
            const farthestFoodX = Matth.max(
